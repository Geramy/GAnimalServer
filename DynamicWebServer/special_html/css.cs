...
namespace DynamicWebServer.special_html
{
    ...
    public static class Make_Special
    {
        public static string MakeJavaScript(string functionName, object[] Enums, string[] Values)
        {
            ...
            {
                if (EnumUtil.GetDescription(TempE) == "src=")
                {
                    tStrOpen = "<" + EnumUtil.GetDescription(special_xml.javascript) + " type=" + EnumUtil.GetDescription(type.javascript) + "src='" + Values[i] + "' >";
                }
                if (EnumUtil.GetDescription(TempE).IndexOf('.') == 0 && VarObjPos != 1)
                {
    ...
    public enum ajaxCalls
    {
        ...
        XMLHttpRequest,
        [StringValueAttribute(".open(,,)")]
        open,
        [StringValueAttribute(".send(null)")]
        send,
    }
    ...
    public enum Specialjs
    {
        [StringValueAttribute("if (")]
        If,
        [StringValueAttribute("function ")]
        function,
    }
}
